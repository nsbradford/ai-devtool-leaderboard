...
pub async fn get_leaderboard_reviews(
    ...
    let selected_col = if view_type == "weekly" {
        "review_count_7d AS review_count"
    } else {
        "review_count_30d AS review_count"
    };
    ...
    let sql = format!(
        "SELECT event_date::text AS event_date, bot_id, {} FROM {} WHERE event_date BETWEEN $1 AND $2 ORDER BY event_date ASC",
        selected_col, view_name
    );
        ...
        .await
    {
        Ok(r) => r,
        Err(e) => {
            let body = Json(json!({"error": format!("Failed to fetch leaderboard review data: {}", e)}));
    ...
    let mut date_groups: BTreeMap<String, Vec<(i64, i64)>> = BTreeMap::new(); // date -> [(bot_id, review_count)]
    ...
    for row in rows {
        ...
        let review_count: i64 = row.try_get("review_count").unwrap_or(0);
        all_bot_ids.insert(bot_id);
        date_groups.entry(date).or_default().push((bot_id, review_count));
    }
    ...
    for (date_index, date) in sorted_dates.iter().enumerate() {
        if let Some(day_data) = date_groups.get(date) {
            for (bot_id, review_count) in day_data {
                let key = bot_id.to_string();
                if let Some(series) = tools.get_mut(&key) {
                    series[date_index] = *review_count;
                }
            }
        }
    }
