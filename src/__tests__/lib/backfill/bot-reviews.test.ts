...
const { upsertBotReviewsForDate } = await import(
  '@/lib/postgres/bot_reviews_daily_by_repo'
);
  ...
  describe('processBotReviewsForDate', () => {
    ...
    it('should process bot reviews successfully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Processing bot reviews for date: 2023-07-14'
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        'Successfully processed 2 bot reviews for 2023-07-14'
      );
    ...
    it('should process bot reviews with specific bot IDs', async () => {
      ...
      expect(upsertBotReviewsForDate).toHaveBeenCalledWith(mockBotReviews);
      expect(consoleSpy).toHaveBeenCalledWith(
        'Processing bot reviews for date: 2023-07-14 (filtering for 3 bots)'
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        'Successfully processed 2 bot reviews for 2023-07-14 (filtering for 3 bots)'
      );
    ...
    it('should handle empty bot reviews array', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Successfully processed 0 bot reviews for 2023-07-14'
      );
    ...
    it('should handle BigQuery errors', async () => {
      ...
      await expect(processBotReviewsForDate('2023-07-14')).rejects.toThrow(
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error processing bot reviews for 2023-07-14:',
        error
      );
    ...
    it('should handle different date formats', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'Processing bot reviews for date: 2023-12-31'
      );
