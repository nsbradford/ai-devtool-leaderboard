...
describe('bigquery', () => {
  ...
  describe('getBotReviewsForDay', () => {
    ...
    it('should fetch bot reviews for a specific date', async () => {
      ...
      vi.mocked(BigQuery).mockReturnValue(
        mockBigQueryInstance as unknown as InstanceType<typeof BigQuery>
      );
      ...
      expect(mockBigQueryInstance.query).toHaveBeenCalledWith({
        query: expect.stringContaining(
          "target_date DATE DEFAULT DATE('2023-07-14')"
        ),
        useLegacySql: false,
      });
    });
    ...
    it('should use specific bot IDs when provided', async () => {
      ...
      expect(mockBigQueryInstance.query).toHaveBeenCalledWith({
        query: expect.stringContaining(
          'bot_id_list ARRAY<INT64> DEFAULT [1, 3, 5]'
        ),
        useLegacySql: false,
      });
    });
    ...
    it('should use all devtools when no bot IDs provided', async () => {
      ...
      expect(mockBigQueryInstance.query).toHaveBeenCalledWith({
        query: expect.stringContaining(
          'bot_id_list ARRAY<INT64> DEFAULT [1, 2]'
        ),
        useLegacySql: false,
      });
    });
    ...
    it('should handle BigQuery errors', async () => {
      ...
      await expect(getBotReviewsForDay('2023-07-14')).rejects.toThrow(
        'BigQuery error'
      );

      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'BigQuery bot reviews query failed:',
        expect.any(Error)
      );
    ...
    it('should handle different date formats correctly', async () => {
      ...
      expect(mockBigQueryInstance.query).toHaveBeenCalledWith({
        query: expect.stringContaining(
          "target_date DATE DEFAULT DATE('2023-12-31')"
        ),
        useLegacySql: false,
      });
    });
