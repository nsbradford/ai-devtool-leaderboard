...
describe('github-api', () => {
  ...
  describe('GitHubApi constructor', () => {
    it('should initialize with valid token', () => {
      ...
      vi.mocked(Octokit).mockReturnValue(
        mockOctokit as InstanceType<typeof Octokit>
      );
    ...
    it('should throw error when GITHUB_TOKEN is missing', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        '[GitHubApi] Missing GITHUB_TOKEN environment variable'
      );
  ...
  describe('fetchRepoMetadata', () => {
    ...
    it('should batch repositories correctly', async () => {
      const repos: RepoFullName[] = Array.from(
        { length: 150 },
        (_, i) => `owner${i}/repo${i}` as RepoFullName
      );
