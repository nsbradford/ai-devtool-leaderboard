...
const MIN_DATE = '2023-07-01';
const MAX_DATE = format(new Date(Date.now() - 24 * 60 * 60 * 1000), 'yyyy-MM-dd');

const shouldFetchData = (startDate: string, endDate: string): boolean => {
  ...
  if (end < minDate || start > maxDate) return false;
...
export default function LeaderboardChart() {
  ...
  const debouncedDateRange = useDebounce(internalDateRange, 500);
  
  const apiDateRange = useMemo(() => {
    if (shouldFetchData(debouncedDateRange.startDate, debouncedDateRange.endDate)) {
      return debouncedDateRange;
    }
    return {
      startDate: MIN_DATE,
      endDate: MAX_DATE
  ...
  const { data: stats, error: statsError, isLoading: statsLoading } = useSWR<LeaderboardData>(
    shouldFetchData(apiDateRange.startDate, apiDateRange.endDate) 
          ...
          <Card>
            <CardHeader>
                        ...
                        <div className="flex flex-col space-y-3 p-4 border-b rounded-t-lg bg-muted/50">
                          ...
                          <div className="space-y-1">
                            {Object.keys(stats.tools)
                              .map((toolId) => ({
                                toolId,
                                displayName: devtools ? devtools.find((dt: DevTool) => dt.id === toolId)?.name || `Tool ${toolId}` : `Tool ${toolId}`,
                                devtool: devtools ? devtools.find((dt: DevTool) => dt.id === toolId) : null,
                              }))
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                  ...
                  <input
                    type="date"
                    value={internalDateRange.startDate}
                    onChange={(e) => setInternalDateRange(prev => ({ ...prev, startDate: e.target.value }))}
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                  ...
                  <input
                    type="date"
                    value={internalDateRange.endDate}
                    onChange={(e) => setInternalDateRange(prev => ({ ...prev, endDate: e.target.value }))}
                    className="px-3 py-2 border border-input bg-background rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent"
                  />
                </div>
              </div>
            ...
            <CardContent>
              {loading || !devtools ? (
                ...
                <div className="h-64 sm:h-96 flex items-center justify-center">
                  <div className="text-muted-foreground">No data available for selected date range</div>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              {loading || !devtools ? (
                ...
                <div className="text-center text-muted-foreground">No ranking data available</div>
              )}
            </CardContent>
          </Card>
        </>
