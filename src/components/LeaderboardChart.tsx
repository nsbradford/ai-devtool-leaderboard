...
export default function LeaderboardChart() {
  ...
  const shouldFetchData: boolean = useMemo(() => {
    const minDate = '2023-07-01';
    const maxDate = format(new Date(), 'yyyy-MM-dd');
    
    const requestedStart = new Date(debouncedDateRange.startDate);
    const requestedEnd = new Date(debouncedDateRange.endDate);
    const minValidDate = new Date(minDate);
    const maxValidDate = new Date(maxDate);
    
    if (requestedStart < minValidDate || requestedEnd > maxValidDate) {
      return false;
    }
  ...
  const { data: stats, error: statsError, isLoading: statsLoading } = useSWR<LeaderboardData>(
    shouldFetchData ? `${baseUrl}/api/leaderboard?${leaderboardParams}` : null,
    fetcher
  );
  ...
  const renderControls = () => {
    const availableTools = stats?.tools ? Object.keys(stats.tools) : [];
    const hasData = stats && devtools;
              ...
              <div className="flex flex-col space-y-3 p-4 border-b rounded-t-lg bg-muted/50">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">
                    {selectedTools.size === 0 ? 'All tools selected' : `${selectedTools.size} of ${availableTools.length} tools selected`}
                  </span>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
                      if (selectedTools.size === availableTools.length) {
                        ...
                        setSelectedTools(new Set(availableTools)); // Select all
                      }
                    }}
                    disabled={!hasData}
                  >
                    {selectedTools.size === availableTools.length ? 'Clear All' : 'Select All'}
                  </Button>
                </div>
                <div className="space-y-1">
                  {hasData ? availableTools
                    .map((toolId) => ({
                      toolId,
                      displayName: devtools.find((dt: DevTool) => dt.id === toolId)?.name || `Tool ${toolId}`,
  ...
  const getToolDisplayName = (toolId: string): string => {
    if (!devtools) return `Tool ${toolId}`;
  ...
  const pageStructure = (
    <div className="container mx-auto p-4 sm:p-6 space-y-6">
      ...
      {/* Chart Section - Always show controls */}
      <Card>
        <CardHeader>
          {renderControls()}
          {renderDateControls()}
        ...
        <CardContent>
          {statsError || devtoolsError ? (
            <div className="text-center text-red-500">
              Error: {statsError?.message || devtoolsError?.message || 'An error occurred'}
            </div>
          ) : statsLoading || devtoolsLoading || !stats || !devtools ? (
